...
def review_with_openai(goal, action, context=None, workspace_files=None):
...
def analyze_test_failure(goal, test_result):
    ...
    try:
        import openai
        client = openai.OpenAI(api_key=api_key)
        # Attempt to read the last 50 lines of server.log if it exists
        server_logs = ""
        if os.path.exists("server.log"):
            with open("server.log", "r") as f:
                lines = f.readlines()
                server_logs = "".join(lines[-50:])
...
Background Server Logs (Last 50 lines):
{server_logs if server_logs else "No background server logs available."}

Analyze the exact root cause of the failure based on these results and the server logs. 
